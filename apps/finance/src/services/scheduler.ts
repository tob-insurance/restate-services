...
async function scheduleNextRun(ctx: ObjectContext) {
  ...
  ctx.console.log(
    `[DEBUG] Current Jakarta Time: ${jakartaYear}-${String(jakartaMonth + 1).padStart(2, "0")}-${String(jakartaDay).padStart(2, "0")} ${String(jakartaHour).padStart(2, "0")}:${String(jakartaMinute).padStart(2, "0")}`
  );
  ...
  ctx.console.log(
    `[DEBUG] Target Jakarta Time (today): ${new Date(targetJakartaMs).toISOString().replace("Z", "")} (config: ${SCHEDULE_CONFIG.hour}:${String(SCHEDULE_CONFIG.minute).padStart(2, "0")})`
  );

  // If target time has passed today in Jakarta, schedule for tomorrow
  if (targetJakartaMs <= jakartaNowMs) {
    ctx.console.log(
      "[DEBUG] Target time has passed today. Scheduling for tomorrow."
    );
  ...
  ctx.console.log(
    `📅 Next run scheduled for: ${targetStr} (in ${Math.round(delayMs / 1000 / 60)} minutes)`
  );
